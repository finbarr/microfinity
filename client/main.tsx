...
import {CartridgeIntro} from './CartridgeIntro';
...
function App(){
  ...
  const playlistEdit=useRef<{roomId:string;revision:number}|null>(null);
  ...
  async function saveName(){const chosen=name.trim();if(!chosen){setError('Enter your name before playing.');return false;}if(guestRef.current?.name===chosen)return true;try{await api('/guest/name',{name:chosen});const updated={...guestRef.current!,name:chosen};setGuest(updated);guestRef.current=updated;localStorage.setItem('microfinity.guest',JSON.stringify(updated));localStorage.setItem('microfinity.name',chosen);return true;}catch(e){setError(`Could not save your name: ${(e as Error).message}`);return false;}}
  ...
  useEffect(()=>{
    if(['create','party-create'].includes(tab)&&!launching&&(!room||room.phase==='lobby'))return;
  ...
  async function createRoom(versions=selected){
    if(!await saveName())return;
    const selection=versions.length?{versions}:{random:{...defaultRandomFilters,seed:crypto.getRandomValues(new Uint32Array(1))[0]}};
    await launch({...selection,settings:soloSettings},true);
  }
