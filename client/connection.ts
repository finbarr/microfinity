...
export function closedConnection(code: number, reason: string): Connection {
  if (reason === 'server-shutdown') return {status:'ended',message:'The server restarted and this party has ended. Saved results and recordings remain available from the arcade.'};
  if (reason === 'room-expired' || code === 4004) return {status:'ended',message:'This party has closed. Return to the arcade to find your saved recordings or start another party.'};
