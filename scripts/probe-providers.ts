...
try{
  ...
  const body=await response.json() as any;report.openai={status:response.status,available:body.data?.map((m:any)=>m.id).filter((s:string)=>/gpt-6-sol|gpt-5-mini|gpt-image|gpt-4.1-mini/.test(s))??[],error:response.ok?null:body.error?.code??'request-failed'};
