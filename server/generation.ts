...
export class GenerationService {
  ...
  private async json(job:Job,model:string,name:string,schema:any,prompt:string,maxTokens=5000){
    ...
    const body={model,input:prompt,max_output_tokens:maxTokens,...(model.startsWith('gpt-5')||model.startsWith('gpt-6')?{reasoning:{effort:'low' as const}}:{}),text:{format:{type:'json_schema' as const,name,schema,strict:true}}};budget.reserve(body,maxTokens);
  ...
  private async run(job:Job,wantArt:boolean){
    ...
    try{
      const model=process.env.CODE_MODEL??'gpt-6-luna',musicModel=process.env.MUSIC_MODEL??model;
