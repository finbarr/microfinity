...
Briefs, game code and repairs default to GPT-6 Sol with high reasoning. Music independently defaults to GPT-5 Mini with low reasoning. `CODE_MODEL` and `MUSIC_MODEL` can override those models; image generation keeps its separate `IMAGE_MODEL` setting.
