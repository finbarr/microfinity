...
import {creationStats} from '../server/stats';import {recordReplayPlay} from '../server/matches';
test('leaderboards isolate rules groups and keep each player best score in the declared direction',async()=>{
...
test('statistics distinguish completed losses, interruption, control methods, replay deduplication and repairs',async()=>{
 ...
 try{
  ...
  await recordReplayPlay(store,'metrics-0',guest.id);await recordReplayPlay(store,'metrics-0',guest.id);
  ...
  assert.deepEqual({...stats.lifecycle,tracked_since:null},{tracked:6,completed:2,active:1,interrupted:1,abandoned:1,technical_failures:1,tracked_since:null});assert.deepEqual(stats.engagement,{completed_parties:2,replayed_parties:1,rematched_parties:1});
