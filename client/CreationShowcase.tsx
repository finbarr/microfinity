...
export function CreationShowcase({job,autoPlay,inParty,onPlay,onRetry,onEdit,onAdd,adding,canAdd,onSound}:{job:any;autoPlay:boolean;inParty:boolean;onPlay:(version:string)=>void;onRetry:()=>void;onEdit:()=>void;onAdd:()=>void;adding:boolean;canAdd:boolean;onSound:()=>void}){
 ...
 useEffect(()=>{let canceled=false;setManifest(null);const version=job.finishedVersion??job.previewVersion;if(version)void fetch(`/api/versions/${version}`).then(r=>{if(!r.ok)throw Error('Preview unavailable');return r.json();}).then(v=>{if(!canceled)setManifest(v.manifest);}).catch(()=>{});return()=>{canceled=true;};},[job.id,job.previewVersion,job.finishedVersion]);
 const icon:Asset|undefined=job.previewIcon??(manifest as (Manifest&{icon?:Asset})|null)?.icon;
 ...
 const hasIconBranch=job.branches?.icon!==undefined||!!icon;
 const steps=[...(hasIconBranch?[['icon','COVER']]:[]),['art','ART'],['music','MUSIC'],['code','GAME']];
 ...
 return <section className={`creation-showcase ${ready?'ready':''}`} aria-label="Your cartridge taking shape" tabIndex={-1} data-nav-focus>
  ...
  <div className="creation-reveal">
   <div className={`creation-art ${art||icon?'arrived':''}`}><span className="creation-label">01 · THE LOOK</span><div className="creation-images">{hasIconBranch&&<div className="creation-icon"><span>CARTRIDGE COVER</span>{icon?<img key={icon.hash} src={icon.url} alt={`Cover for ${job.title??'your game'}`}/>:<div className="creation-icon-pending" aria-hidden="true">{job.branches.icon==='failed'?'!':'✦'}</div>}</div>}<div className="creation-sprite"><span>GAME ART</span>{art?<img key={art.hash} src={art.url} alt={`Artwork for ${job.title??'your game'}`}/>:<div className="creation-placeholder"><span className="pixel-spark" aria-hidden="true">✦</span>{job.branches.art==='failed'?'Artwork needs another try':'Drawing a little character…'}</div>}</div></div><p>{icon&&art?'Cover and game art are ready.':art?'Meet your new little world.':icon?'Your cover is ready.':'Your artwork will appear here.'}</p></div>
  ...
  <ol className="creation-steps" aria-label="Creation progress">{steps.map(([branch,label])=>{const state=job.branches?.[branch]??(branch==='icon'&&icon?'ready':'pending'),done=['ready','reused','skipped'].includes(state);return <li key={branch} className={done?'done':state==='failed'?'failed':''}><span aria-hidden="true">{done?'✓':state==='failed'?'!':'·'}</span>{label}<small>{done?'Ready':state==='failed'?'Try again':state==='working'?'Making…':'Up next'}</small></li>;})}</ol>
