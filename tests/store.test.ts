...
test('durable guest identity, immutable runtime versions, and idempotent results survive reopening',async()=>{
  ...
  try{
    ...
    let guards=0;await assert.rejects(()=>store.putVersion(source,code,{...meta,id:'expired-publication'},[],null,{},guest.id,undefined,undefined,()=>{if(++guards===3)throw new Error('Work deadline before commit');}),/deadline/);
