...
for(const file of files.filter(f=>!requested.length||requested.includes(f.slice(0,-3)))){
  ...
  const started=performance.now();let ticks=0,deterministicRuns=0,restores=0,renderChecks=0;const scores:number[]=[],outcomes:Record<string,number>={},coverage:any[]=[];
  try{
    for(let episode=0;episode<100;episode++){
      ...
      const sampledSteps:any[]=[];let saved:any,saveIndex=0,rng=episode+13,steps=0;
      while(!status.done&&steps<8000){
        ...
        if(episode<4)sampledSteps.push({edges,dt,event});
        if(episode<4&&steps===20){saved=vm.call('save');saveIndex=sampledSteps.length;}
      ...
      if(episode<4){
        vm.call('init',config);for(const j of sampledSteps)vm.call('step',j.edges,j.dt,j.event);assert.equal(hash(JSON.stringify(vm.call('save'))),digest);deterministicRuns++;
        if(saved){vm.call('restore',saved);for(const j of sampledSteps.slice(saveIndex))vm.call('step',j.edges,j.dt,j.event);assert.equal(hash(JSON.stringify(vm.call('save'))),digest);restores++;}
      }
      coverage.push({seed:config.seed,players:count,difficulty,policy,steps,done:status.done,hash:episode<4?digest:undefined});
    }
    const elapsed=performance.now()-started;report.games.push({game:meta.id,episodes:100,ticks,milliseconds:Math.round(elapsed),stepsPerSecond:Math.round(ticks/(elapsed/1000)),deterministicRuns,restores,renderChecks,outcomes,minScore:Math.min(...scores),maxScore:Math.max(...scores),meanScore:scores.reduce((a,b)=>a+b,0)/scores.length,coverage});console.log(`${meta.title}: 100 episodes, ${ticks} steps, ${Math.round(ticks/(elapsed/1000))} steps/s, ${deterministicRuns} deterministicRuns match`);
  }finally{vm.dispose();}
}
