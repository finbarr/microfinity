...
export async function seed(store:Store){
  ...
  for(const file of (await readdir('games')).filter(f=>f.endsWith('.ts')).sort()){
    ...
    try{const {meta,audio}=vm.call('meta'),saved=existing.get(meta.id);await store.putVersion(source,code,meta,saved?.assets??[],saved?.music??null,{...saved?.provenance,kind:'reference',art:saved?.assets.length?'ready':'pending',music:saved?.music?'ready':'pending'},undefined,undefined,audio);console.log(`Ready: ${meta.title}`);}finally{vm.dispose();}
  }
}
