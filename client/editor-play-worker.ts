...
let input = new PreviewInput([]), result: any, seat = 'p0', ticks = 0;
...
self.onmessage = async ({data}) => {
  try {
    if (data.type === 'load') {
      ...
      input = new PreviewInput(Array.from({length: players}, (_, i) => `p${i}`));
      ...
      input.enqueue(seat, data.edges);
    } else if (data.type === 'seat') {
      input.releaseAll();
      if (Array.from({length: players}, (_, i) => `p${i}`).includes(data.seat)) {seat = data.seat; post();}
