...
export async function compileIsolated(source:string,signal?:AbortSignal):Promise<{code:string;meta:any;assets:string[];audio:any}>{
  ...
  return new Promise((resolvePromise,reject)=>{
    ...
    // Preflight executes all four party sizes (up to ten private attempts).
    // The enclosing generation deadline can still cancel this process sooner.
    const timer=setTimeout(()=>done(new Error('Compilation/preflight exceeded 60 seconds')),60000);
