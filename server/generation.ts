    ...
    try{
      ...
      const codeTask=(async()=>{
        ...
        for(let attempt=0;attempt<this.limits.codeAttempts;attempt++){
          const result=await work(()=>this.json(job,model,'cartridge',{type:'object',properties:{source:{type:'string'}},required:['source'],additionalProperties:false},base+repair,this.limits.codeOutputTokens));source=result.source;
